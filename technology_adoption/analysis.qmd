...
Countries have ISO codes instead of proper names. 
Let's fix that.
...
Arbitrarily, I pick those related to ships
```{python}
...
we'd expect from these variables in a dataset stretching back to the 1950s - no one collected data on air transport back then, of course.
```{python}
labels = [ 
